...
export function registerStopTool(pi: ExtensionAPI, runtime: HerdrToolRuntimeController, name: string): void {
  pi.registerTool({
    ...
    description: "Gracefully stop, or explicitly force-close, only a freshly identity-verified current-active-branch owned subagent. Captures any pending result before temporary cleanup when possible. Worktree cleanup is separate and defaults to retain; remove_if_safe requires captured handoff evidence, parent review, objective Git evidence, cleanliness, no live or unknown panes, and identity-matched removal. This tool never authorizes dirty discard.",
