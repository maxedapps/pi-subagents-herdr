...
export async function assertEphemeralRuntimeLayout(
  ...
  } else if (files.sessionDirectory !== undefined) {
    throw new Error("Claude/Codex ephemeral runtime must not claim a Pi sessions child");
  }
  ...
  const expectedEntries = files.sessionDirectory === undefined ? ["system.md"] : ["sessions", "system.md"];
...
export async function createEphemeralRuntimeFiles(
  ...
  try {
    ...
    const files: EphemeralRuntimeFiles = harness === "pi"
      ? { directory, systemPrompt, sessionDirectory: join(directory, "sessions") }
    ...
    try {
      ...
      if (!entries.every((entry) => entry === "system.md" || entry === "sessions")) throw new Error("Unexpected construction-time runtime entry");
