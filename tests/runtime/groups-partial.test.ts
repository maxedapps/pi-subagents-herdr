...
import { agent, pane, tab } from "../herdr/fixtures.ts";

function fakeClient() {
  let creates = 0; let panes: PaneInfo[] = [{ ...pane, pane_id: "w1:anchor", terminal_id: "term_anchor", tab_id: "w1:group", focused: false }]; let agents: AgentInfo[] = [];
  const closedPanes: string[] = []; const closedTabs: string[] = [];
  const client = {
    async createTab(input: any) { creates += 1; assert.equal(input.focus, false); await new Promise((resolve) => setTimeout(resolve, 10)); return { tab: { ...tab, tab_id: "w1:group", focused: false }, rootPane: panes[0]! }; },
    ...
    async listPanes() { return panes; }, async listAgents() { return agents; },
    async closePane(paneId: string) { closedPanes.push(paneId); panes = panes.filter((item) => item.pane_id !== paneId); },
    async closeTab(tabId: string) { closedTabs.push(tabId); },
  } as unknown as HerdrRequestClient;
  return { client, get creates() { return creates; }, get panes() { return panes; }, set panes(value: PaneInfo[]) { panes = value; }, set agents(value: AgentInfo[]) { agents = value; }, closedPanes, closedTabs };
}
