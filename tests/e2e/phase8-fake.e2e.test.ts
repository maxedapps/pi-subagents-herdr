...
for (const scenario of matrix) {
  test(`deterministic fake E2E: ${scenario.id}`, async () => {
    ...
    const result = await execFileAsync(process.execPath, ["--import", "tsx", "--test", ...pattern, scenario.file], {
...
test("deterministic source/package load and real-Herdr operator script remain no-model by default", async () => {
  const smoke = await execFileAsync(process.execPath, ["--import", "tsx", "scripts/smoke-load.ts"], { cwd: process.cwd(), encoding: "utf8", timeout: 15_000 });
  assert.match(smoke.stdout, /clean-load ok/);
  const real = await execFileAsync(process.execPath, ["scripts/e2e-herdr-smoke.mjs", "--harness", "pi"], {
